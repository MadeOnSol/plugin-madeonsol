...
import type { TokenSurges, TokenSurgesParams } from "../client.js";
...
export const tokenSurgesAction: Action = {
  ...
  handler: async (
    ...
    const text = message.content?.text || "";
    const params: TokenSurgesParams = { limit: 20 };
    const mintMatch = text.match(MINT_RE);
    if (mintMatch) params.mint = mintMatch[1];
    if (/\b(revival|revivals|revived|reviving|dormant|woke|waking|wake up|came back)\b/i.test(text)) params.kind = "revival";
    else if (/\b(surge|surges|surging|breakout|breakouts|running hard|pumping)\b/i.test(text)) params.kind = "surge";
    if (params.kind === "surge") {
      if (/\bbreakout/i.test(text)) params.tier = "breakout";
      else if (/\bstrong\b/i.test(text)) params.tier = "strong";
      else if (/\bearly\b/i.test(text)) params.tier = "early";
    }
    if (/\b(clean|no flags|without (risk )?flags|unflagged)\b/i.test(text)) params.only_clean = "1";
    if (/\b(hit rate|hit-rate|stats|statistics|how often|success rate)\b/i.test(text)) params.stats = "1";
    ...
    if (usdMatch) {
      ...
      if (Number.isFinite(n) && n > 0) params.min_mc_usd = n;
    }
    ...
    const data = result.data as TokenSurges;
