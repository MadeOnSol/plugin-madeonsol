...
// ── Token surges & revivals (token momentum fires, 1.24) ──
...
/**
 * One token momentum fire — the `token:surge` / `token:revival` WS + webhook payload and a
 * `getTokenSurges` row minus `outcome`. Both kinds share one shape: `tier`, `baseline_*`,
 * `mc_multiple`, `mc_change_3m_pct` are null on revivals; `dormant_hours`, `prev_mc_usd`,
 * `mc_vs_prev_multiple` are null on surges.
 */
export interface TokenSurgeEvent {
  ...
  birth_source: "sniper" | "deployer" | "first_seen" | null;
  age_seconds: number | null;
  market_cap_usd: number;
  liquidity_usd: number | null;
  liquidity_to_mc_ratio: number | null;
  price_usd: number | null;
  /** Surge only: launch MC; `late` = engine saw the token late, multiple not applied. */
  baseline_mc_usd: number | null;
  baseline_source: "launch" | "late" | null;
  ...
  /** Burst tape since birth / revival. Counts are null when no tape covers the window yet (`available:false`). */
  tape: {
    since: string | null;
    available: boolean;
    source: "candles" | "wallet_trades" | null;
    buys: number | null;
    sells: number | null;
    trades: number | null;
    ...
    /** Only where the mint is in trade coverage (`wallet_data_available`) — never an inferred zero. */
    unique_buyers: number | null;
    unique_wallets: number | null;
    trades_per_wallet: number | null;
    wallet_data_available: boolean;
  };
  kol: { buyers: number; buys: number; sells: number; names: string[] };
  early_buyers: { count: number; bundled: number; cohort_sol: number | null; sold: number; sniper_wallets: number };
  deployer: {
    wallet: string | null;
    tier: string;
  ...
  /** Empty = no flag raised, NOT verified clean. */
  risk_flags: TokenSurgeRiskFlag[];
  detail_url: string;
  enrichment_available?: boolean;
}

/** +1 h outcome on `getTokenSurges` rows ≥ 65 min old. `priced_after_1h:false` = no candle in the hour, NOT zero. */
...
/** Per-(kind, tier) hit-rate row inside `getTokenSurges({ stats: "1" })`. */
...
/** Response of `getTokenSurges(params)` — token momentum fires, newest first. */
export interface TokenSurges {
  events: Array<TokenSurgeEvent & { id: number; outcome: TokenSurgeOutcome | null }>;
  pagination: { limit: number; count: number; has_more: boolean; next_since: string | null; next_before: string | null };
  filters: Record<string, unknown>;
  /** Present only when `stats` was requested. */
  stats?: { days: number; note: string; rows: TokenSurgeStatsRow[] };
  /** WebSocket pointer for the `token:surges` channel (events `token:surge` / `token:revival`). */
  stream?: { channel: string; url?: string; [k: string]: unknown };
  /** Live engine thresholds (surge / revival / shared / risk_flags / tiers). */
...
export interface TokenSurgesParams {
  ...
  /** Comma list — rows carrying ANY of these flags are dropped. */
 ...
 *  never a synthesized row. Returned by `getDeployerAsOf`. */
export interface DeployerAsOf {
  is_deployer: boolean;
  wallet: string;
...
export interface DeployerRewardsTopRecipient {
  ...
  is_self: boolean;
  is_social_pda: boolean;
  social: { platform: number; user_id: string } | null;
}

/**
 * pump.fun creator-fee rewards for a wallet, answered two ways that are never
 * merged: `collected` (what actually reached the wallet) and `attributed`
 * (every payout on the tokens it deployed, split `to_self`/`to_others`).
 * Returned by `getDeployerRewards`.
 */
export interface DeployerRewards {
  wallet: string;
  is_deployer: boolean;
 ...
 */
export function copyTradeSignalsQuery(
  params?: { subscription_id?: string | number; rule_id?: string | number } & Record<string, string | number | undefined>,
): string {
  ...
  if (params) {
    ...
    for (const [k, v] of Object.entries(rest)) if (v !== undefined) qs.set(k, String(v));
  }
...
export class MadeOnSolClient {
   ...
   */
  getDeployerAsOf(wallet: string, date?: string) {
    const qs = date !== undefined ? `?date=${encodeURIComponent(date)}` : "";
    return this.restRequest<DeployerAsOf>("GET", `/deployer-hunter/${encodeURIComponent(wallet)}/as-of${qs}`);
  }
   ...
   */
  getDeployerRewards(wallet: string) {
    return this.restRequest<DeployerRewards>("GET", `/deployer-hunter/${encodeURIComponent(wallet)}/rewards`);
  }
  ...
  updateWebhook(id: number, params: { url?: string; events?: string[]; is_active?: boolean }) {
    return this.restRequest("PATCH", `/webhooks/${id}`, params);
  }
   ...
   * lapses or you pass `{ rotate: true }`, which replaces it (the previous value
   * keeps working for 60 s). `expires_at` / `next_refresh_at` are always `null`;
   * the response also carries `rotated` (boolean) and `lifetime` (string). A
   * `4001` close means "mint again", never a timer. Authenticate the handshake
   * with `Authorization: Bearer <token>`.
   */
  getStreamToken(opts?: { rotate?: boolean }) {
    return this.restRequest("POST", "/stream/token", opts?.rotate ? { rotate: true } : undefined);
  }
  ...
  relabelWatchlist(walletAddress: string, label: string | null) {
  ...
  getSniperRecent(params?: { deployer_tier?: string; min_bond_rate?: number; since?: string; watchlist?: boolean; limit?: number }) {
    const qs = new URLSearchParams();
    if (params) {
      for (const [k, v] of Object.entries(params)) {
  ...
  getSniperByDeployer(wallet: string, params?: { limit?: number }) {
   ...
   */
  getTokenSurges(params?: TokenSurgesParams) {
    const qs = new URLSearchParams();
    for (const [k, v] of Object.entries(params ?? {})) {
    ...
    const query = qs.toString() ? `?${qs.toString()}` : "";
    return this.restRequest<TokenSurges>("GET", `/tokens/surges${query}`);
  }
  ...
  getTokenTopTraders(mint: string, params?: { limit?: number; sort?: "pnl" | "roi"; window_days?: number; min_bought_sol?: number }) {
    const qs = new URLSearchParams();
    if (params?.limit !== undefined) qs.set("limit", String(params.limit));
    ...
    const query = qs.toString();
    return this.restRequest("GET", `/tokens/${encodeURIComponent(mint)}/top-traders` + (query ? "?" + query : ""));
  }
