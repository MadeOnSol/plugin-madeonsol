...
export const tokenUnlocksAction = {
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        if (mint)
            params.mint = mint;
        if (/\b(biggest|largest|top)\b/i.test(text))
            params.sort = /\b(pct|percent|% of supply|share of supply)\b/i.test(text) ? "largest_pct" : "largest_usd";
        if (/\bstreamflow\b/i.test(text))
            params.program = "streamflow";
        else if (/\bjupiter\b/i.test(text))
            params.program = "jupiter_lock";
        else if (/\bbonfida\b/i.test(text))
            params.program = "bonfida_vesting";
