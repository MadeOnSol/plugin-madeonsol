...
const usd = (v) => (v == null ? "n/a" : `$${v.toLocaleString(undefined, { maximumFractionDigits: 2 })}`);
...
export const tokenFeeClaimsAction = {
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        if (mint)
            params.mint = mint;
        if (/\bsocial\b|\bx account\b|\btwitter\b/i.test(text))
            params.type = "social_claim";
        else if (/\bdistributions?\b|\bpayouts?\b/i.test(text))
            params.type = "distribution";
        else if (/\bcreator claims?\b|\bvault claims?\b/i.test(text))
            params.type = "creator_claim";
